...
#smb     <- smbinning(data_train, y="OUTCOME", x=var)
#df.iv   <- rbind(df.iv, data.frame(Var=var, IV=smb$iv))
#data_train <- smbinning.gen(data_train, smb, chrname = var_bin)
#data_test  <- smbinning.gen(data_test , smb, chrname = var_bin)
```
...
var <- 'DEM'
smb <- smbinning(data_train, y="OUTCOME", x=var, p = 0.05)
lst.smb[[var]] <- smb
smb$ctree
smb$iv
...
```{r IV8, echo=TRUE}
var <- 'SHIPTYPE'
smb <- smbinning.factor(data_train, y="OUTCOME", x=var)
lst.smb[[var]] <- smb
df.iv <- rbind(df.iv, data.frame(Var=var, IV=smb$iv))
smb$iv
```
...
```{r IV9, echo=TRUE}
var = 'ORDER_DT_HOUR'
smb <- smbinning.factor(data_train, y="OUTCOME", x=var, maxcat = 50)
df.iv <- rbind(df.iv, data.frame(Var=var, IV=smb$iv))
smb$iv
...
```{r IV12, echo=TRUE}
var <- 'USERGROUP'
smb <- smbinning.factor(data_train, y="OUTCOME", x=var, maxcat = 100)
lst.smb[[var]] <- smb
smb$iv
smb$ivtable[, c("Cutpoint", "WoE")]
...
summary(logitMod)
data_test$predicted <- predict(logitMod, data_test, type="response")  # predicted scores

plotROC(data_test$OUTCOME, data_test$predicted)
```


```{r IV12356343533, echo=TRUE}
plot(sort(data_test$predicted))
```
...
df <- data.frame(OUTCOME=data_test$OUTCOME, predicted=data_test$predicted) 
b <- seq(0, 1, 0.05)
...
```{r useful1}
i <- !is.na(data_test$APPLIED) & !is.na(data_test$COST) & !is.na(data_test$predicted) & data_test$predicted >= 0.50
sum(data_test[i,]$APPLIED) / sum(1.1*data_test[i,]$COST)
```

Net Margin of these orders

```{r useful2}
 (sum(data_test[i,]$ORIGINAL) - 1.1*sum(data_test[i,]$COST))/sum(data_test[i,]$ORIGINAL)
```

Total Net Margin 

```{r useful3}
 (sum(data_test$ORIGINAL) - 1.1*sum(data_test$COST))/sum(data_test$ORIGINAL)
```


```{r IV1234ew5633, echo=TRUE}
i <- !is.na(data_test$predicted) & data_test$predicted >= 0.50
sum(i)
sum(i)/length(i)
```


# A Logistic Regression Model for Buyers

```{r IV127767531, echo=TRUE}
i <- data_train$NEW_CHANNEL == 'Affiliate'
models[["Registrants"]] <- glm(OUTCOME ~  #WOE_BIN_STATE +
                                          WOE_USERGROUP +
                                          WOE_BIN_MONTHS_SINCE_REG +
                                          WOE_ELIGIBLES +
                                          WOE_BIN_VERIFIEDSALARY +
...
```{r saveobjects}
save(models, lst.smb, file = "predict.Rdata")
```